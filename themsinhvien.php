      ...
      <div class="container-fluid">
        ...
        <?php
            ...
            if(isset($_POST['btn-submit'])){
              ...
              if($mssv==""||$hoten==""){
                ...
                if($clb==""){
                  echo "<script>alert('Vui lòng chọn câu lạc bộ');</script>";
                } else{
                  $db=new db();
                  $sql="INSERT INTO sinhvien (MSSV, HoTen, SDT, MaCLB) VALUES ('".$mssv."', '".$hoten."', '".$sdt."', '".$clb."')";
                  $kq=$db->query_execute($sql);
                  if($kq){
                      echo "<script>alert('Thêm sinh viên thành công');</script>";
                      echo "<script> window.location = 'index.php'; </script>";
                  } else{
                      echo "<script>alert('Lỗi thêm sinh viên');</script>";
                  }
                }
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="form-row" >
                ...
                <div class="value">
                    <input class="input--style-6" type="number" name="txt_masv" value=""> 
                </div>
            </div>
            <div class="form-row" >
            ...
            <div class="form-row" >
                ...
                <div class="value">
                  <select class="input-group" name='txt_CLB'>
                  <option value="" selected>Chọn câu lạc bộ</option>
                    <?php
                      $data1=new db();
                      $sqlCLB="SELECT * FROM caulacbo";
                      $result=$data1->select_to_array($sqlCLB);
                      foreach ($result as $clb)
                      {
                          echo "<option value='".$clb['MaCLB']."'>".$clb['TenCLB']."</option>";                                      
                      }
                    ?>
                  </select>
                </div>
            ...
            <button class="btn btn--radius-2 btn--blue-2" type="submit" name="btn-submit">Thêm sinh viên</button>
            </div> 
        </form>
      </div>
      <!-- /.container-fluid -->
