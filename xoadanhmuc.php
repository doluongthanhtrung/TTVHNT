...
<body class="bg-dark">
  ...
  <div class="container">
    <div class="card card-login mx-auto mt-5">
      ...
      <div class="card-body">
        <form method="post">
            <Label>Bạn muốn xóa danh mục: <?php echo $tendanhmuc;?></Label>
            <button class="btn btn-primary btn-block" type="submit" name="btn-submit">Xóa</button>
        </form>
        <?php
        if(isset($_POST['btn-submit'])){
            $data=new db();
            $sql="DELETE FROM danhmuciso WHERE MaISO='".$madanhmuc."'";
        ...
        <div class="text-center">
          <a class="d-block small mt-3" href="danhmuciso.php">Hủy</a>
        </div>
      </div>
    </div>
  </div>
