...
<ul class="sidebar navbar-nav">
    ...
    <div id="content-wrapper">
      ...
      <div class="container-fluid">
        ...
        <!-- Breadcrumbs-->
        <ol class="breadcrumb">
          <li class="breadcrumb-item active">Thêm câu lạc bộ</li>
        </ol>
        ...
        <?php
            ...
            if(isset($_POST['btn-submit'])){
              $maclb=$_POST['txt_maCLB'];
              $tenclb=$_POST['txt_tenCLB'];
              if($maclb==""||$tenclb==""){
                  echo "<script>alert('Vui lòng nhập thông tin đầy đủ');</script>";
              } else{
                
                  $db=new db();
                  $sql="INSERT INTO caulacbo (MaCLB, TenCLB) VALUES ('".$maclb."', '".$tenclb."') ";
                  $kq=$db->query_execute($sql);
                  if($kq){
                      echo "<script>alert('Thêm câu lạc bộ thành công');</script>";
                      echo "<script> window.location = 'caulacbo.php'; </script>";
                  } else{
                      echo "<script>alert('Lỗi thêm câu lạc bộ thất bại');</script>";
                  }
              }
            } 
        ?>
        ...
        <div class="form-row" >
                <div class="name">Mã câu lạc bộ</div>
                <div class="value">
                    <div class="input-group" data-validate="Vui lòng nhập dữ liệu">
                      <input class="input--style-6" type="text" name="txt_maCLB" value=""> 
                    </div>
                </div>
            </div>
            <div class="form-row" >
                <div class="name">Tên câu lạc bộ</div>
                <div class="value">
                    <input class="input--style-6" type="text" name="txt_tenCLB" value=""> 
                </div>
            </div>
            <div class="card-footer">
            <button class="btn btn--radius-2 btn--blue-2" type="submit" name="btn-submit">Thêm câu lạc bộ</button>
            </div> 
        </form>
      </div>
      <!-- /.container-fluid -->
